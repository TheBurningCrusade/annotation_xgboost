...
class TreeModel {
 public:
  ...
  class Node {
   public:
    ...
    // 这里的feature index 指得应该是对特征进行编号之后的特征号,
    // (1U << 31) - 1U 表示的是将32位数的最高位置成0其他位都为1,
    // 即: 01111111111111111111111111111111, 实现的功能就是去sindex_的除了最高位
    // 之后的值是多少
   ...
   private:
     ...
     */
    //一个节点当他是中间节点的时候它需要存储条件判断，当他是叶子节点的时候它需要存权重
    union Info{
      float leaf_value;
    ...
    // parent_他的最高位代表该节点是父节点的左孩子还是右孩子，如果parent_的最高位为1那么该节点就是父节点的左孩子
    int parent_;
    ...
    Info info_;
    // set parent
    // 如果该节点是父节点的左孩子那么最高位置成1，反之直接将父节点的索引赋值给他
 ...
 protected:
  ...
  // stats of nodes 
  // 每个节点都有一个上统计信息存储在stats里面，他和树中的节点个数相等
 ...
 public:
   ...
   * \param new leaf value
   */
  // 这个就是递归的将一个节点下的所有节点删除然后再将自己变成叶子节点
 ...
 public:
  ...
  // nid 是一个节点的id，这里的所有的id都是用vector的小标标识的，而且都是统一的
  // 比如nodes的id可以用在leaf_vector上，这里的leaf_vector存储了该叶子节点的一些额外
  // 的信心，而这些信息的大小就是size_leaf_vecotor所制定的
  ...
  inline void InitModel(void) {
    ...
    nodes.resize(param.num_nodes);
    stats.resize(param.num_nodes);
    // leaf_vector 是一个展平的vector，其中每一个节点存储的信息所占有的空间的大小为size_leaf_vector
  ...
  inline int MaxDepth(void) {
    int maxd = 0;
    // 由下面这个for循环可以推出，nodes的前param.num_roots个节点都是根节点
 ...
 public:
  ...
  inline int GetLeafIndex(const FVec&feat, unsigned root_id = 0) const {
    ...
    while (!(*this)[ pid ].is_leaf()) {
      // 根据节点的编号找到该节点的特征号, pid就是节点的编号，然后找到特征号
      unsigned split_index = (*this)[pid].split_index();
      // 根据特征号到实例中去找该特征对应的特征值
