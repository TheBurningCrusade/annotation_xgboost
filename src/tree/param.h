...
struct TrainParam{
  ...
  inline bool need_forward_search(float col_density = 0.0f) const {
    // 输出都是0
    // std::cout<< "default direction: " << default_direction << std::endl;
...
struct SplitEntry{
   ...
   */
  // 当loss_chg不等于最好的策略时（翻译),当前存储的小的分割索引更有优势
  // 也就是说当前分割索引如果大的话，会按照new_loss_chg>loss_chg的情况进行替换
  // 如果当前索引小的话，
