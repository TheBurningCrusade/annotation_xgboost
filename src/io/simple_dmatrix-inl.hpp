...
class DMatrixSimple : public DataMatrix {
 public:
  ...
  inline void LoadText(const char *uri, bool silent = false, bool loadsplit = false) {
    ...
    LibSVMParser parser(dmlc::InputSplit::Create(uri, rank, npart, "text"), 16);
    this->Clear();
    while (parser.Next()) {
   ...
   * 尝试打开稀疏矩阵的二进制文件，如果没有的话在就返回false，
   * 有的话进行稀疏矩阵初始化
   */
  ...
  // data fields
  // row_ptr_和row_data_ 就将整个矩阵就读入进来了，可以是通过读取我们原始的文本文件
  // 也可以一下子将转换成矩阵的二进制文件读取出来,这两个变量就实现了CRS矩阵的存储方式
