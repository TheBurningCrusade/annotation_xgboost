...
class FMatrixS : public IFMatrix {
 public:
  ...
  virtual float GetColDensity(size_t cidx) const {
    // buffered_rowset_.size 表示的是行数, 即特征的个数
    // 小括号中表示的一列特种中非空的个数，所以nmiss表示的是一列特征中为空值的个数
  ...
  inline static void LoadBinary(utils::IStream &fi,
    ...
    // 这里应该是多了一个哨兵，用来判断矩阵的行数到了末尾了， 不对；
    // 这多出来的一个元素应该是最后的一个元素，他用来表明一共有多少个元素
    out_ptr->resize(nrow + 1);
    // 读取的是每一行矩阵第一个非空元素的列索引数组
