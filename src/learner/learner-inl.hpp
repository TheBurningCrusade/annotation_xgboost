...
class BoostLearner : public rabit::Serializable {
 public:
  ...
  inline void SetCacheData(const std::vector<DMatrix*>& mats) {
    ...
    /* 这里两个for循环意思是在训练和验证数据集中有可能验证数据集之间或者验证数据集和测试
     * 数据集之间相同，这里只会将不同的数据集的预测数据进行缓冲，不会重复缓冲*/
 ...
 protected:
  ...
  inline void PredictRaw(const DMatrix &data,
    ...
    // ndata的长度是所有的preds的大小，包括leaf_value 和leaf_vector
 ...
 protected:
  ...
  /*!这个CacheEntry应该是用来在训练是缓冲一个行训练实例的，它的元素mat_可能
   * 是用来存储特定的matrix，而now_row_是该matrix的行索引
   */
  
  // 这个函数用来查询给定matrix的的buffer_offset_，它具体的功能还不知是什么?
