...
class GBTree : public IGradBooster {
 public:
  ...
  virtual void DoBoost(IFMatrix *p_fmat,
    ...
    // 通过num_output_group的判断可以看出来， new_trees中第一个vector的大小和
    // num_output_group的大小是相同的
  ...
  virtual void Predict(IFMatrix *p_fmat,
    ...
    // 为每一个线程分配一个FVec，即操作一个实例（一行向量)的结构
    thread_temp.resize(nthread, tree::RegTree::FVec());
    for (int i = 0; i < nthread; ++i) {
      // 将数据都出事化为-1，见实现，数据的长度就是num_feature
      thread_temp[i].Init(mparam.num_feature);
    }
    std::vector<float> &preds = *out_preds;
    // num_output_group 应该标识的是一个实例的预测结果可以有几个？？ 
    // stride 应该表示可以预测的总的结果的个数
    const size_t stride = info.num_row * mparam.num_output_group;
    // stride 英文的意思是步幅,那么可以推断一个步幅就是所有实例在num_output_group
    // 下的总个数, preds
    ...
    iter->BeforeFirst();
    while (iter->Next()) {
      // 这里表示我们拿到一个batch，注意batch应该是几个实例合起来叫一个batch，即连续的几行数据在矩阵中
      const RowBatch &batch = iter->Value();
      // parallel over local batch
      // nsize 标识batch中实例的个数即行的个数
      const bst_omp_uint nsize = static_cast<bst_omp_uint>(batch.size);
      #pragma omp parallel for schedule(static)
      for (bst_omp_uint i = 0; i < nsize; ++i) {
        const int tid = omp_get_thread_num();
        // 分配一行数据的存储空间，这里要转化成dense的形式
        tree::RegTree::FVec &feats = thread_temp[tid];
        // ridx应该表示的是在整个batch群中，现在的batch的第i行数据的位置
        // 其中base_rowid表示的是当前的batch的偏移位置
        int64_t ridx = static_cast<int64_t>(batch.base_rowid + i);
        utils::Assert(static_cast<size_t>(ridx) < info.num_row, "data row index exceed bound");
        // loop over output groups
        // 应该是对于每一个实例我们都需要预测它num_output_group次，这是为什么？
        // ridx * mparam.num_output_group + gid 表示的就是我们需要预测的那个实例数据需要存储到
        // 的地方
        for (int gid = 0; gid < mparam.num_output_group; ++gid) {
          this->Pred(batch[i],
 ...
 protected:
  ...
  inline void InitUpdater(void) {
    ...
    // 这里默认的参数有两个一个grow_colmaker,另一个是prune
    while (pstr != NULL) {
      // 依据不同的字符串，创建不通过的updaters
      updaters.push_back(tree::CreateUpdater(pstr));
      // 对不同的updaters按照配置进行参数的出事化
  ...
  BoostNewTrees(const std::vector<bst_gpair> &gpair,
    ...
    this->InitUpdater();
    // create the trees
    // 这里通过在二分类问题中的输出也印证num_parallel_tree=1,像注释所说的那样他只会在随机森林中在被赋予大于1的值
    for (int i = 0; i < tparam.num_parallel_tree; ++i) {
      // std::cout << "alal" << tparam.num_parallel_tree << std::endl;
    ...
    // update the trees
    // 这里的updater是按照grow_colmaker,和prune两步来执行的，这个循环就是负责做这个的
    for (size_t i = 0; i < updaters.size(); ++i) {
      // 每个步骤都要更新里面所有的树
  ...
  inline void Pred(const RowBatch::Inst &inst,
                   int64_t buffer_index, // 全量batch情况下的index
                   int bst_group, //第几个group,范围是0 到 num_ouput_group
                   unsigned root_index, //该实例的所属的根节点
                   tree::RegTree::FVec *p_feats, //一个存储一行稀疏实例的dense 行实例
                   float *out_pred,// 预测结果的存储 
                   size_t stride, // 全部的实例中一共要预测的个数,即num_output_group * 实例的总个数 
                   unsigned ntree_limit) {
    size_t itop = 0;
    float  psum = 0.0f;
    // sum of leaf vector 
    ...
    if (bid >= 0 && ntree_limit == 0) {
      ...
      // pred_buffer的存储单元是以size_leaf_vector+1 为单位的，其中每个单元的第一个元素存储的是
      // 一个实例在各个树上预测的总的值, 其他的size_leaf_vector个元素到底是存储的什么？
    for (int i = 0; i < mparam.size_leaf_vector; ++i) {
  ...
  if (itop != trees.size()) {
    ...
    for (size_t i = itop; i < trees.size(); ++i) {
      if (tree_info[i] == bst_group) {
        // 对已一个节点，它在BoostInfo中只有一个root_index值，但是他却可以用于不同的树?
        int tid = trees[i]->GetLeafIndex(*p_feats, root_index);
        // psum用来相加一个实例在一批树的叶子节点的值
        psum += (*trees[i])[tid].leaf_value();
        for (int j = 0; j < mparam.size_leaf_vector; ++j) {
          // vec_psum 用来相加一个实例在一批树上叶子节点的向量
...
struct ModelParam {
     ...
     */
    // buffer_index 代表的是当前batch下的第几行数据
